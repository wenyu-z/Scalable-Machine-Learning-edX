...
def parsePoint(line):
    ...
    label = np.float(SplitFeatures[0])
    SplitFeatures = SplitFeatures[1:]
    SplitFeatures = map(lambda x: np.float(x), SplitFeatures)
    features = SplitFeatures
    my_lp = LabeledPoint(label, features)
    return my_lp
...
def calcRMSE(labelsAndPreds):
    ...
    """
    rmse = np.sqrt(labelsAndPreds.map(lambda (x, y): squaredError(x, y)).mean())
    return rmse
...
def gradientSummand(weights, lp):
    ...
    """
    summand = np.dot((DenseVector.dot(weights, lp.features) - lp.label), lp.features)
    return summand
...
def twoWayInteractions(lp):
    ...
    interimList = itertools.product(features,features)
    newfeatures = [x[0]*x[1] for x in interimList]
    allfeatures = np.hstack((features, newfeatures))
    newlp = LabeledPoint(label, allfeatures)
    return newlp
print np.hstack(([2,3],itertools.product([2,3], repeat = 1)))
